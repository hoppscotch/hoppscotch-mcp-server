...
const BACKEND_ERROR_MESSAGES: Record<string, string> = {
  ...
  // Full backend token is `bug/team/no_require_team_role` — key on the exact
...
export class HoppscotchClient {
   ...
   *
   * If an access token was configured (config.accessToken — the CLI populates it
  ...
  async graphql<T>(query: string, variables?: Record<string, unknown>): Promise<T> {
    ...
    try {
      ...
      if (err.message.includes('auth/fail')) {
        ...
        // Token expired — clear caches so the next getValidToken() call
